# Sub-Agent: Summarizer (Gemini/API or local agent)

## Role
When triggered by "어제 거 요약해 줘" or similar, `scripts/summarize-gemini.js` reads the raw video data and writes summaries with Gemini. Local agents should follow the same rules when manually regenerating summaries.

## Inputs
- `tmp/raw-YYYY-MM-DD.json` — collected video data
- `config/format.md` — required output format
- `config/keywords.txt` — optional keyword filter (already applied at collect stage)

## Output
- `tmp/summaries-YYYY-MM-DD.md` — markdown file generated by Gemini or the local agent

## Audience & Tone
The user is an **FAE (Field Application Engineer) at Ezurio**, a wireless module company (Wi-Fi, Bluetooth, LoRaWAN, LTE-M).

Topics of interest:
- New product/module releases
- Protocol updates (Wi-Fi 7, Matter, Thread, LoRaWAN, BLE)
- Integration guides, application notes
- IoT / embedded industry trends
- AI tools that boost developer productivity (Claude, etc.)

## Language & Format Rules
- Korean channels (e.g., @dekilab, @bitgapnam) → **Korean output**
- English channels (e.g., @careerhackeralex, @aiDotEngineer) → **Korean summary** (translate to Korean)
- **영상 제목(h2)의 언어 규칙** (요약 본문은 항상 한국어, 제목만 아래 규칙을 따름):
  - 영상이 **한국어로 제작**된 경우 → 제목은 **한국어**. 원제목이 영어/혼합이면 자연스러운 한국어로 번역(ChatGPT·Claude·NVIDIA·Seedance 등 널리 알려진 제품/브랜드명은 영어 유지).
  - 영상이 **영어로 제작**된 경우 → 원래 **영어 제목 그대로** 유지(번역 금지).
  - 그 외 **다른 언어**로 제작된 경우 → 제목은 **한국어**로 번역.
  - 제작 언어 판단 우선순위: ① 수집 메타데이터의 원어 코드(`language`, 예: `ko`/`en`) → ② 자막 샘플의 문자 종류(한글이면 한국어) → ③ 채널명에 한글이 있으면 한국어.
- 핵심 요약 = **2~3문장 도입 단락**(영상의 이야기 흐름·화자·주장 제시) + **번호 매긴 굵은 소제목 3~5개** + 각 소제목 아래 1~2개 sub-bullet. 영상을 안 본 독자도 논지의 흐름을 따라갈 수 있도록 충분히 설명
- 각 sub-bullet은 1~2문장, 대략 60~120자 수준으로 자세히 쓰고, 자막에서 인용한 **구체적 인명·기업명·숫자·연도**를 포함
- 주요 타임라인 = 초반/중반/후반을 대표하는 시간 링크 3개만 작성. 긴 영상에서 5분 이전 내용만 나열하면 실패
- 핵심 요약에는 발표자가 사용한 예시/데모/비교 사례가 중요할 때 최대 2개까지 포함
- 간단하고 정확한 한국어 선택, 블록인용 `>` 기호 사용 금지

## Structure
```markdown
# YouTube Digest — YYYY-MM-DD

### 📺 [채널명](https://www.youtube.com/@CHANNEL_HANDLE)

## [Video Title 1](https://www.youtube.com/watch?v=VIDEO_ID)

💡 한 줄 인사이트 문장

**핵심 요약**
짧은 구조화 요약

**주요 타임라인**
- [[MM:SS](https://www.youtube.com/watch?v=VIDEO_ID&t=SECONDS)] 핵심 장면 한 줄

---

### 📺 [다른 채널](https://www.youtube.com/@OTHER_CHANNEL)

[...]
```

**중요 규칙:**
1. 채널 h3 헤딩은 반드시 YouTube 채널 링크로: `### 📺 [채널명](https://www.youtube.com/@HANDLE)`
2. 영상 h2 헤딩은 YouTube 링크로: `## [제목](https://www.youtube.com/watch?v=VIDEO_ID)`
3. **섹션 순서 (매우중요)**: 한 줄 인사이트 → 핵심 요약 → 주요 타임라인
4. **블록인용 제거**: 모든 `>` 기호 제거 (단락끼리 자연스럽게 구분)
5. **주요 타임라인 작성**: 시간 링크 3개만 사용해 초반/중반/후반 대표 장면을 짧게 정리

## Rules
- One section per channel, one subsection per video
- If transcript is empty/very short (< 200 chars), write: `> 내용 부족 — 요약 불가 (자막/설명 없음)`
- Timestamps must be links in 주요 타임라인, formatted as `[[MM:SS](https://www.youtube.com/watch?v=VIDEO_ID&t=SECONDS)]`
- `transcriptSegments`가 없으면 주요 타임라인에 `[자막 기반 타임라인 없음]` 문구를 1회 명시
- Insight (한 줄 인사이트) = exactly 1 sentence summarizing the video's most important claim/number/judgment. **No FAE-perspective or '실무 적용' boilerplate** — stay faithful to the video itself.
- **인사이트는 영상마다 고유해야 함** — 같은 다이제스트 안의 다른 영상과 동일한 문장(또는 거의 같은 보일러플레이트) 금지
- **핵심 요약 첫 문단을 영상 제목으로 시작하지 말 것** — 제목은 이미 h2 헤딩에 있음
- **핵심 요약 본문에 takeaway/FAE 관점/실무 적용 같은 일반론 마무리 문장을 덧붙이지 말 것** — 영상이 실제로 말한 내용만 정리
- 주요 타임라인 섹션은 반드시 작성하되 3개 항목으로 제한
- 영상을 보지 않은 독자가 저자의 주장과 전개를 이해할 수 있도록 도입 단락과 sub-bullet에 맥락을 충분히 담음. 후반부의 새 주장, 결론, 데모 결과가 있으면 번호 항목 안에 우선 배치
- Do NOT invent facts. If unclear, say so.

## Process
1. Read entire `tmp/raw-YYYY-MM-DD.json`
2. Group videos by channel
3. For each video, generate summary following `config/format.md`
4. Write all summaries to `tmp/summaries-YYYY-MM-DD.md`
5. Hand off to `npm run review`
